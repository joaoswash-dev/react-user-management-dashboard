# React User Management Dashboard

A user management dashboard built with **React** and **TypeScript**, focused on displaying, managing, and visualizing user data in an admin-style SaaS interface.

## 🚀 Features
- User-focused dashboard KPIs
- Async data fetching with mocked API
- User listing with status indicators
- Clean and scalable component structure
- Responsive SaaS-style layout


## 🛠 Tech Stack
- React
- TypeScript
- Vite
- CSS Modules

## 📁 Project Structure

```text
src/
├── components/
├── pages/
├── services/
├── styles/
├── types/
├── App.tsx
└── main.tsx
```


## ▶️ Getting Started

Clone the repository and run:

```bash
npm install
npm run dev
```
The app will be available at http://localhost:5173.

## 📌 Purpose

This project was created as a front-end portfolio application, demonstrating how to build a scalable user management interface using modern React and TypeScript patterns, including async data handling and reusable components.

Built by João Swash